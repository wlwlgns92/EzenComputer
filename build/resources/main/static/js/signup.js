$(function(){
    ...
    $("#memberPasswordconfirm").keyup(function(){
        ...
        if(!memberPasswordj.test(memberPasswordconfirm)) {
            $("#memberPasswordconfirmcheck").html("숫자', '문자', '특수문자' 포함 , '최소 8문자~16글자 허용");
        } else if(memberPassword != memberPasswordconfirm) {
            $("#memberPasswordconfirmcheck").html("서로 비밀번호가 다릅니다.");
        } else {
    ...
    $("#sample4_postcode").keyup(function() {
            ...
            if(address1.indexOf("/") != -1) {
                $("#memberAddresscheck").html("주소에 '/' 특수문자 포함 불가"); return false;
            }
            if(address1 != null) {
